...
describe('login', () => {
  ...
  cy.get('.oxd-table-card > .oxd-table-row').then(($rows) => {
      ...
      cy.wrap($row).find(':nth-child(2) > div').then(($cell) => {
        ...
        if ($cell.text().includes('testikMM1')) {
          if (index > 0) { // Skontroluj, či riadok nie je prvý
            ...
            cy.wrap($row).find('.oxd-table-cell-actions > :nth-child(1) > .oxd-icon').click();
    ...
    cy.get('.oxd-text--toast-message').should('be.visible').should('contain.text', 'Successfully Deleted')
  })
  })
})
